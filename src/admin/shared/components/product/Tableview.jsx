...
const Tableview = (props) => {
  ...
  const description = (rowData) => {
    return <div className="p-2" dangerouslySetInnerHTML={{ __html: rowData.Product_Description }} />;
  };

  const name = (rowData) => {
    return <div>{rowData.Product_Name}</div>;
  };
  ...
  const getOption = async (key) => {
    const filterOptions = await getFilterOptions(key.field);
