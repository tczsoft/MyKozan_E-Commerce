...
const Checkout = () => {
    ...
    return (
        <div className="min-h-[70vh] lg:py-0 py-10 max-w-[70rem] mx-auto mt-32">
                    ...
                    <div className="grid md:grid-cols-2 grid-cols-1 overflow-hidden bg-white rounded-lg shadow-xl">
                        <div className="p-1 md:p-6">
                            ...
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button onClick={() => { setFormdata({}); setIsEditMode(false); setShowNewAddressModal(true); }} className="flex items-center text-sm transition-colors text-[#00712D] hover:text-[#346347]" >
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {shippingdata && shippingdata.length > 0 ? (
                                    shippingdata.map((address, index) => (
                                        <div key={index} onClick={() => setSelectedAddress(address)} className={`relative p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 ${selectedAddress?._id === address._id ? 'border-[#00712D] bg-[#e9fff2]' : 'border-gray-200 hover:border-[#00712D]' }`} >
                                            <div className="flex items-start justify-between">
                                                ...
                                                <div className="flex space-x-2">
                                                    <button onClick={(e) => { e.stopPropagation(); handleEditAddress(address); }} className="p-2 text-gray-400 transition-colors hover:text-blue-600" >
                                                        <MdModeEdit className="w-5 h-5" />
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); handleDeleteAddress(address._id); }} className="p-2 text-gray-400 transition-colors hover:text-red-600" >
                        ...
                        <div className="p-6">
                                    ...
                                    cartItems.map((item) => (
                                        <div key={item.productId._id} className="flex items-center justify-between">
                                            <div className="flex items-center">
                                                <img src={`${apiurl()}/${item.productId.Images[0]}`} alt={item.productId.Product_Name} className="object-cover w-16 h-16 rounded-md" />
                                                <div className="ml-4">
                            ...
                            <div className="p-6 mt-4 bg-white">
                                <button onClick={handlePlaceOrder} disabled={loading || !selectedAddress} className={`w-full flex justify-center items-center px-6 py-3 rounded-md text-white text-lg font-medium transition-all duration-200 ${loading || !selectedAddress ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#00712D] hover:bg-[rgb(227,135,52)]' }`} >
                                    {loading ? (
                                        <>
            ...
            {showNewAddressModal && (
                <ShippingForm visible={showNewAddressModal} setVisible={setShowNewAddressModal} formdata={formdata} handlechange={handleAddressChange} handlesave={handlesave} loading={loading} />
            )}
        </div>
    );
};
